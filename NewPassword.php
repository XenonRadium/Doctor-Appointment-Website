...
if(isset($_POST['Submit'])){
    ...
    $cpassword = filter_input(INPUT_POST, 'confirmpassword');
    if($password !== $cpassword){
        $errors['password'] = "Confirm password not matched!";
    }else{
        $code = 0;
        $email = $_SESSION['email']; //getting this email using session
        $update_pass = "UPDATE usertable SET `userPassword` = '$password' WHERE `userEmail` = '$email'";
      ...
      <div class="form-content">
        <form method="post" action="">
          <div class="form-group">
            ...
            <input class="input-box" id="username" type="text" name="password" required="required" />
          </div>
          <div class="form-group">
            <label for="username">New Confirm Password</label>
            <input class="input-box" id="username" type="text" name="confirmpassword" required="required" />
          </div>
          <div class="form-group">
